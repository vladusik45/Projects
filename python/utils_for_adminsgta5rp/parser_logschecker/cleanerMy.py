    ...
    with open(filename, 'w') as file:
        if matching_accounts:
            ...
            for account_num, account_characters in matching_accounts:
                if character['faction'] == 'Русская мафия': faction1 = 'RM'
                if character['faction'] == 'Армянская мафия': faction1 = 'AM'
                if character['faction'] == 'Мексиканская мафия': faction1 = 'MM'
                if character['faction'] == 'Японская мафия': faction1 = 'YAK'
                if character['faction'] == 'Итальянская мафия': faction1 = 'LCN'
                if character['faction'] == 'Marabunta Grande': faction1 = 'MG'
                if character['faction'] == 'Los Santos Vagos': faction1 = 'LSV'
                if character['faction'] == 'Bloods': faction1 = 'BSG'
                if character['faction'] == 'The Ballas Gang': faction1 = 'ESB'
                if character['faction'] == 'The Families': faction1 = 'FAM'
                file.write(f"Аккаунт: {account_num}\n")
                for character in account_characters:
                    file.write(f"offwarn {character['ID']} twink {faction1} + \n")
...
def check_nickname_endings(matching_accounts, ignored_faction):
    endings = {
        'Итальянская Мафия': ['ossa', 'eri', 'ino', 'ini', 'ito', 'chi', 'te', 'li', 'ne', 'lo', 'no', 'tti', 'igo'],
            ...
            if line.startswith('ACC:'):
                if account_num is not None:
                    for character in account_characters:
                        ...
                        rank = character['rank']
                        if faction == ignored_faction and rank is not None and rank > 4:  # проверка фракции на соответствие ignored_faction
...
output_file = f'Twinks{filename}'
nicknamechecker = f'Nicknames{filename}'
